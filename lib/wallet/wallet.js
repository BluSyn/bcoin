...
class Wallet extends EventEmitter {
  ...
  /**
   * Fund a transaction with inputs without a lock.
   * @private
   * @see MTX#selectCoins
   * @see MTX#fund
   */
  ...
  /**
   * Fill a transaction with as many of passed coins
   * as possible
   * @see MTX#selectCoins
   * @see MTX#fill
   * @param {MTX} mtx - _Must_ be a mutable transaction.
  ...
  async fill(mtx, coins, options, force) {
    const unlock = await this.fundLock.lock(force);
    try {
      return await this._fill(mtx, coins, options);
    } finally {
      unlock();
    }
  }
  ...
  async _fill(mtx, coins, options) {
    let accountIndex = 0;
    if (options.account && typeof options.account === 'string')
      accountIndex = await this.getAccountIndex(options.account);

    // pick random address from options.addresses or generate new one
    const addressCount = options.addresses ? options.addresses.length : 0;
    let address;
    if (addressCount) {
      const index =
      ...
      address = await this.receiveAddress(accountIndex);
    }

    if(!options.selection)
      options.selection = 'reverseValue';
  ...
  * @param {(String|Number)?} options.account - If no account is
  * specified, coins from the entire wallet will be filled.
  * @param {(Integer)?}  options.threshold - largest coin value
  * to include in the sweep
  * @param {String|Buffer} passphrase - passphrase for wallet
  ...
  */
  ...
  async sweep(options = {}, passphrase) {
    ...
    try {
      return await this._sweep(options, passphrase);
    } finally {
      unlock();
    }
  }
  ...
  async _sweep(options, passphrase) {
    let account = 0;
    if (options.account)
      account = options.account;

    const txs = [];
    let coins = await this.getCoins(account);

    // if received threshold option, filter for only
    // coins with a value that is smaller than threshold
    if (options.threshold) {
      assert(Number.isSafeInteger(options.threshold));
      assert(options.threshold >= -1);
      coins = coins.filter(coin => coin.value < options.threshold);
    }

    if (options.rate == null)
      options.rate = await this.wdb.estimateFee(options.blocks);

    this.logger.info('Starting sweep of %d coins', coins.length);
    let totalCoins = 0;
    // Continue sweeping while there are still available coins.
    // At the end of each loop iteration coins already in tx
    // should be locked and coins array set to be only unlocked coins
    while(coins.length) {
      // create blank mtx
      const mtx = new MTX();
      await this.fill(mtx, coins, options, true);
      ...
      totalCoins += tx.inputs.length;

      // lock the tx coins so we can use the rest
      // to continue sweeping
      this.txdb.lockTX(tx);
      // set coins list to only unlocked coins
      const prevCoinsLength = coins.length;
      coins = await this.txdb.filterLocked(coins);

      // make sure that coin count is decreasing
      // so we don't get stuck in infinite loop
      assert(coins.length < prevCoinsLength);
      txs.push(tx);
    }

    if (txs.length > 0)
      this.logger.info('Sending %d sweep transactions with %d coins',
        txs.length, totalCoins);
