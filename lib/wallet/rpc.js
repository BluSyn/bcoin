...
class RPC extends RPCBase {
  ...
  async sweep(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'sweep "passphrase" ( options )');
    ...
    if (options) {
      ...
      rate = valid.ufixed('feeRate', 8);
      // default account -1 means "entire wallet"
      account = valid.int('account', -1);
      threshold = valid.ufixed('threshold', 8, null);
      // Ignored if account is set
      addresses = valid.array('addresses');
    }

    await wallet.sweep({ account, rate, threshold, addresses }, passphrase);
