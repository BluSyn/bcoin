...
class MTX extends TX {
   ...
   */
   async fill(coins, options) {
    ...
    // unless otherwise set, we want to sort and select coins by value
...
class CoinSelector {
  ...
  fromOptions(options) {
    ...
    if (options.threshold != null) {
      assert(
        ...
        'threshold must be safe integer'
      );
      assert(options.threshold >= 0, 'threshold must be positive');
  ...
  init(coins) {
    ...
    switch (this.selection) {
      ...
      case 'value':
        ...
        break;
      case 'reverseValue':
        this.coins.sort(sortReverseValue);
        break;
      default:
  ...
  async selectFill() {
    ...
    while(this.index < this.coins.length) {
      ...
      // skip the coin if the value of the coin
      // is larger than target threshold
      if (this.threshold && coin.value >= this.threshold)
        continue;
